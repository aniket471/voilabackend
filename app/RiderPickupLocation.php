...
class RiderPickupLocation extends Model
{
    ...
    public static function updateRiderLocationContinuously($request)
    {
        ...
        if (isset($arr[self::rider_id])) {

            if (self::where(self::rider_id, $arr[self::rider_id])->first()) {

                $updateLocation = DB::update('update rider_pickup_location set rider_pickup_lat=? ,rider_pickup_long=?,rider_pickup_address=? where rider_id=?',[$arr[self::rider_pickup_lat],$arr[self::rider_pickup_long],$arr[self::rider_pickup_address],$arr[self::rider_id]]);
                if($updateLocation)
                ...
                $riderLocation[self::rider_id] = $arr[self::rider_id] ?? 0;
                $riderLocation[self::rider_pickup_lat] = $arr[self::rider_pickup_lat] ?? 0;
                $riderLocation[self::rider_pickup_long] = $arr[self::rider_pickup_long] ?? 0;
                $riderLocation[self::rider_pickup_address] = $arr[self::rider_pickup_address]??"";
